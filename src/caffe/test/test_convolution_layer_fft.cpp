...
TYPED_TEST(ConvolutionLayerTestFFT, TestSetup) {
  ...
  shared_ptr<Layer<Dtype> > layer(
      new ConvolutionLayerFFT<Dtype>(layer_param, true));
  ...
  layer.reset(new ConvolutionLayerFFT<Dtype>(layer_param, true));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
...
TYPED_TEST(ConvolutionLayerTestFFT, TestGradient) {
  ...
  ConvolutionLayerFFT<Dtype> layer(layer_param, true);
